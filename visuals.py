...
def plot_wealth_trajectory(df_F=None):
    ...
    theoretical_x_arr = np.array(theoretical_x, dtype=float)
    nav_20_arr = np.array(nav_20, dtype=float)

    # ============================================================
    # AI 強週期帶
    # ------------------------------------------------------------
    # 說明：
    # 這裡以既有「野心上限 20%」作為基準航線，
    # 再依照 30% / 35% / 40% 相對於 20% 的成長溢價放大。
    #
    # 這樣做的好處：
    # 1. 不破壞你原本手動校準過的注資 / 槓桿 / 路徑形狀。
    # 2. AI 強週期會自然位於野心路徑上方。
    # 3. 它是「右尾加速情境」，不是新的考核基準。
    # ============================================================
    def build_ai_cycle_path(rate):
        premium = ((1 + rate) / 1.20) ** np.maximum(0, theoretical_x_arr - anchor_x)
        return (nav_20_arr * premium).tolist()

    nav_ai_30 = build_ai_cycle_path(0.30)
    nav_ai_35 = build_ai_cycle_path(0.35)
    nav_ai_40 = build_ai_cycle_path(0.40)
    ...
    fig.add_trace(go.Scatter(
        ...
        name='🧠 AI 強週期中軸 35%',
        mode='lines',
        line=dict(color='rgba(126, 34, 206, 0.75)', width=2.4, dash='dot'),
        line_shape='spline',
        hoverinfo='skip'
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        name='🟢 基準目標',
        mode='lines',
        line=dict(color='#166534', width=3.0),
        line_shape='spline',
        hoverinfo='skip'
    ))
    ...
    if df_F is not None and not df_F.empty:
        ...
        if date_col and '實質NAV' in df_real.columns:
            ...
            if not df_real.empty:
                ...
                df_real['frac_year'] = df_real['dt'].apply(date_to_frac_year)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name='⚡ 實際戰線光暈',
                    mode='lines',
                    line=dict(color='rgba(245, 158, 11, 0.20)', width=9),
                    line_shape='spline',
                    hoverinfo='skip',
                    showlegend=False
                ))
    ...
    for x, label in events:
        fig.add_vline(
            ...
            line_width=1,
            line_dash='dot',
            line_color='rgba(100,116,139,0.50)',
            layer='below'
        )
        fig.add_annotation(
            x=x,
    ...
    for x, y in zip(theoretical_x, nav_175):
        if int(np.floor(x)) in key_years:
            fig.add_annotation(
                x=x,
                y=y,
                text=frac_year_to_quarter_label(x),
                showarrow=False,
                yshift=16,
                font=dict(size=12, color='#166534', family=modern_font)
            )

    # ============================================================
    # 每日 Hover 雷達
    # ============================================================
    daily_dates = pd.date_range(start='2025-07-01', end='2040-12-31', freq='D')
    hover_df = pd.DataFrame({'dt': daily_dates})
    hover_df['date_label'] = hover_df['dt'].dt.strftime('%Y-%m-%d')
    hover_df['frac_year'] = hover_df['dt'].apply(date_to_frac_year)
    ...
    # AI 強週期帶每日內插
    hover_df['ai_30'] = np.interp(hover_df['frac_year'], theoretical_x, nav_ai_30)
    hover_df['ai_35'] = np.interp(hover_df['frac_year'], theoretical_x, nav_ai_35)
    hover_df['ai_40'] = np.interp(hover_df['frac_year'], theoretical_x, nav_ai_40)

    # 實際 NAV 嚴格對應真實資料日，不做內插
    if has_real:
        real_nav_map = dict(zip(df_real['date_str'], df_real['nav_m']))
    ...
    hover_df['ai_band_text'] = hover_df.apply(
        lambda row: f"{row['ai_30']:.2f}M – {row['ai_40']:.2f}M",
        axis=1
    )
    ...
    # 其他日期用基準線作為捕捉高度。
    if has_real:
        real_date_set = set(df_real['date_str'])
        hover_df['hover_y'] = hover_df.apply(
            lambda row: row['real_nav']
            if row['date_label'] in real_date_set and pd.notna(row['real_nav'])
            else row['exp_175'],
            axis=1
        )
    else:
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate=(
            ...
            '🛡️ 大盤防線：%{customdata[5]:.2f}M<br><br>'
            '🧠 AI 強週期帶：%{customdata[6]}<br>'
            '🧠 AI 中軸 35%：%{customdata[7]:.2f}M'
            '<extra></extra>'
        )
    ))
    ...
    fig.update_layout(
        title=dict(
            text=(
                "<b>NEGENTROPIC ATARAXIA 個人資產戰略航線圖</b><br>"
                "<span style='font-size:14px; color:#64748B;'>"
                "主視角：2026–2030｜路徑：300 → 500 → 1000 → 自由區｜AI 強週期帶：30–40%"
                "</span>"
            ),
            font=dict(size=20, family=modern_font),
            x=0.5,
            xanchor='center',
            y=0.98,
            yanchor='top'
            ...
            dict(
                ...
                buttons=[
                    dict(
                        ...
                        method='relayout',
                        args=[{
                            'xaxis.range': [2025.75, 2030.5],
                            'yaxis.range': [1, 25]
                        }]
                    ),
                    dict(
                        label='🛡️ 窒息期 2026–2027',
                        method='relayout',
                        args=[{
                            'xaxis.range': [2025.75, 2027.6],
                            'yaxis.range': [1, 8]
                        }]
                    ),
                    dict(
                        ...
                        method='relayout',
                        args=[{
                            'xaxis.range': [2027.5, 2030.5],
                            'yaxis.range': [3, 25]
                        }]
                    ),
                    dict(
                        label='🏔️ 自由區 2030–2040',
                        method='relayout',
                        args=[{
                            'xaxis.range': [2030, 2040.5],
                            'yaxis.range': [8, 650]
                        }]
                    ),
                    dict(
                        ...
                        method='relayout',
                        args=[{
                            'xaxis.range': [2025.75, 2040.5],
                            'yaxis.range': [0, 650]
                        }]
                    ),
                    dict(
                        label='🗺️ 傳統全景 2025–2040',
                        method='relayout',
                        args=[{
                            'xaxis.range': [2024.5, 2040.5],
                            'yaxis.range': [0, 75]
                        }]
                    ),
                ]
