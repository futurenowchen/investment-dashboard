...
def plot_wealth_trajectory(df_F=None):
    ...
    # 7. 起點紫點: 2026/04 穩態基準點 (分岔錨點)
    fig.add_trace(go.Scatter(
        x=[2026 + anchor_offset],
        y=[2.887],
        name='穩態基準點',
        mode='markers',
    ...
    for ev in events:
        ...
        if ev['symbol'] == 'star':
            fig.add_trace(go.Scatter(
                ...
                y=[ev['y_data']],
                mode='markers',
                marker=dict(symbol='star', size=12, color=ev['color']),
                hoverinfo='skip',
                showlegend=False
            ))
    ...
    # 車貸/分期結束標註 (跟隨錨點偏移)
    fig.add_annotation(
        x=2027 + anchor_offset,
        y=3.9,
        ax=-40,
        ay=40,
        text="<b>2027/05 車貸結束</b><br>+10K/月",
        showarrow=True,
        arrowhead=2,
        arrowcolor="#2CA02C",
        arrowwidth=1.5,
    ...
    fig.add_annotation(
        x=2028 + anchor_offset,
        y=4.8,
        ax=30,
        ay=40,
        text="<b>2027/07 分期結束</b><br>+2.8K/月",
        showarrow=True,
        arrowhead=2,
        arrowcolor="#2CA02C",
        arrowwidth=1.5,
    ...
    fig.update_layout(
        title=dict(
            ...
            font=dict(size=16, family=MODERN_FONT),
            x=0.5,
            xanchor='center',
            y=0.98,
            yanchor='top'
