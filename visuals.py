...
def plot_wealth_trajectory(df_F=None):
    """繪製 NEGENTROPIC ATARAXIA 財富路徑導航圖 (極簡防重疊優化版)"""
    ...
    # 高低錯位陣列 (Staggered Layout) 確保文字水平不碰撞
    y_high = 0.95
    y_low = 0.88
    ...
    # --- 底部里程碑區塊 (上下交錯防擁擠) ---
    # 使用 yref="paper" 懸掛在 X 軸下方，利用 y=-0.12 與 y=-0.22 錯位
    ms_y1 = -0.12
    ms_y2 = -0.22
    ...
    fig.update_layout(
        ...
        margin=dict(t=150, b=150, l=50, r=50), # 頂部和底部保留巨大呼吸空間
