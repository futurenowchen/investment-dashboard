...
if not df_C.empty:
    ...
    with c2:
        ...
        # 進度條
        try:
            target = safe_float(df_c.loc['短期財務目標', col_val])
            gap = safe_float(df_c.loc['短期財務目標差距', col_val])
            if target > 0:
                curr = target - gap
...
# 3. 交易紀錄 (重點修正區域：不使用 style.format)
st.header('3. 交易紀錄與淨值')
...
with t1:
    if not df_D.empty:
        ...
        # 3. 統計
        total = df_calc['淨收／支出'].apply(safe_float).sum()
        col_a, col_b = st.columns(2)
        col_a.metric("篩選淨額", fmt_str_money(total))
        col_b.caption(f"共 {len(df_calc)} 筆")
        ...
        if not df_calc.empty:
            st.caption(f"📅 區間: {df_calc['dt'].min().date()} ~ {df_calc['dt'].max().date()}")
...
with t2:
    if not df_E.empty:
        ...
        with c_sel: 
            sel_s = st.multiselect("篩選股票", stocks, default=stocks, key='pnl_s', label_visibility="collapsed")
        with c_all:
            st.markdown('<div style="height: 28px"></div>', unsafe_allow_html=True) # Spacer
            if st.button("全選"): 
                del st.session_state['pnl_s']; st.rerun()
        with c_clr:
            st.markdown('<div style="height: 28px"></div>', unsafe_allow_html=True)
            if st.button("清除"): 
                st.session_state['pnl_s'] = []; st.rerun()
        
        if sel_s: df_calc = df_calc[df_calc['股票'].isin(sel_s)]
        
        total = df_calc['已實現損益'].apply(safe_float).sum()
...
with t3:
    if not df_F.empty:
        ...
        st.plotly_chart(px.line(df_calc.sort_values('dt'), x='dt', y='nav', title='NAV 走勢'), use_container_width=True)
        
        with st.expander("詳細數據"):
            df_view = df_calc.sort_values('dt', ascending=False).drop(columns=['dt', 'nav']).copy()
            df_view['日期'] = df_view['日期'].apply(fmt_str_date)
...
st.markdown('---')
# 4. 財富藍圖 (文章卡片式呈現 - 解決 ValueError)
st.header('4. 財富藍圖')
if not df_G.empty:
    # 這裡不使用 st.dataframe，改用迴圈生成 Markdown 卡片
    # 這樣可以完美避開任何 "string to float" 的錯誤
    try:
        for i, row in df_G.iterrows():
            # 嘗試獲取欄位，如果欄位名不同也能容錯
            level = row.get('階層') or row.get(df_G.columns[0])
            money = row.get('美金金額範圍') or row.get(df_G.columns[1])
            twd = row.get('約當台幣') or row.get(df_G.columns[2])
            desc = row.get('財富階層意義') or row.get(df_G.columns[3])
            time_est = row.get('以年報酬率18–20%推估所需時間') or (row.get(df_G.columns[4]) if len(df_G.columns)>4 else "")
            ...
            with st.container():
                ...
                if time_est:
                    st.success(f"🚀 推估時間: {time_est}")
                st.divider()
    except Exception as e:
        st.error(f"無法解析財富藍圖表格: {e}")
        st.write(df_G) # 最後手段：直接印出原始表格 (這不會報錯)
else:
    st.warning("讀取不到財富藍圖資料")
