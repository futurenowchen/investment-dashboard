...
if not df_C.empty:
    try:
        ...
        c_code, emoji, txt_col = colors.get('安全') # default
        ...
        with c2:
            ...
            if t_val > 0:
                ...
                st.progress(prog)
                st.caption(f"目前: {curr:,.0f} / 目標: {t_val:,.0f} (差: {gap_val:,.0f})")
            else:
    ...
    if not df_A.empty:
        ...
        for c in num_cols:
            if c in df_A_show.columns:
                df_A_show[c] = df_A_show[c].apply(lambda x: f"{safe_float(x):,.0f}")
...
with c2:
    if not df_B.empty and '市值（元）' in df_B.columns:
        try:
            ...
            df_chart = df_B[~df_B['股票'].astype(str).str.contains('總資產|Total', na=False)]
...
# 定義簡單的格式化函式 (避免 style.format 崩潰)
def fmt_comma(x): return f"{safe_float(x):,.0f}"
def fmt_date(x): 
...
with tab1:
    if not df_D.empty:
        df_D_view = df_D.copy()
        # 排序
        if '日期' in df_D_view.columns:
            df_D_view['dt'] = pd.to_datetime(df_D_view['日期'], errors='coerce')
            df_D_view.sort_values('dt', ascending=False, inplace=True)
            df_D_view['日期'] = df_D_view['dt'].dt.strftime('%Y-%m-%d').fillna(df_D_view['日期'])
            df_D_view.drop(columns=['dt'], inplace=True)
        
        # 篩選
        if '動作' in df_D_view.columns:
            cats = df_D_view['動作'].unique().tolist()
            sel_cats = st.multiselect('篩選動作', cats, default=cats, key='cf_filter')
            df_D_view = df_D_view[df_D_view['動作'].isin(sel_cats)]
        
        # 格式化欄位
        for c in ['淨收／支出', '累積現金']:
            if c in df_D_view.columns: df_D_view[c] = df_D_view[c].apply(fmt_comma)
            
        st.dataframe(df_D_view, use_container_width=True, hide_index=True)

with tab2:
    if not df_E.empty:
        df_E_view = df_E.copy()
        # 排序
        date_col = next((c for c in df_E_view.columns if '日期' in c), None)
        if date_col:
            df_E_view['dt'] = pd.to_datetime(df_E_view[date_col], errors='coerce')
            df_E_view.sort_values('dt', ascending=False, inplace=True)
            df_E_view[date_col] = df_E_view['dt'].dt.strftime('%Y-%m-%d').fillna(df_E_view[date_col])
            df_E_view.drop(columns=['dt'], inplace=True)

        # 篩選
        if '股票' in df_E_view.columns:
            stocks = df_E_view['股票'].unique().tolist()
            c_sel, c_btn1, c_btn2 = st.columns([4, 1, 1])
            with c_sel: sel_stocks = st.multiselect('篩選股票', stocks, default=stocks, key='pnl_stocks', label_visibility="collapsed")
            with c_btn1: 
                if st.button('全選', key='pnl_all'): 
                    st.session_state.pop('pnl_stocks', None)
                    st.rerun()
            with c_btn2: 
                if st.button('清除', key='pnl_none'): 
                    # Multiselect 清除較難直接操作，這裡給出提示或簡單重整
                    pass 
            
            if sel_stocks:
                df_E_view = df_E_view[df_E_view['股票'].isin(sel_stocks)]
        
        # 格式化
        for c in ['已實現損益', '投資成本', '帳面收入']:
            if c in df_E_view.columns: df_E_view[c] = df_E_view[c].apply(fmt_comma)
            
        st.dataframe(df_E_view, use_container_width=True, hide_index=True)

with tab3:
    if not df_F.empty:
        df_F_view = df_F.copy()
        # 繪圖需要數字
        if '實質NAV' in df_F_view.columns and '日期' in df_F_view.columns:
            df_F_view['nav_num'] = df_F_view['實質NAV'].apply(safe_float)
            df_F_view['dt'] = pd.to_datetime(df_F_view['日期'], errors='coerce')
            
            fig = px.line(df_F_view.sort_values('dt'), x='dt', y='nav_num', title='NAV 趨勢')
            st.plotly_chart(fig, use_container_width=True)
            
            # 表格顯示 (字串格式)
            df_F_view.sort_values('dt', ascending=False, inplace=True)
            df_F_view['日期'] = df_F_view['dt'].dt.strftime('%Y-%m-%d').fillna(df_F_view['日期'])
            for c in ['實質NAV', '股票市值', '現金']:
                if c in df_F_view.columns: df_F_view[c] = df_F_view[c].apply(fmt_comma)
            
            with st.expander("詳細數據"):
                st.dataframe(df_F_view.drop(columns=['nav_num', 'dt']), use_container_width=True)
